...
class DatabaseManager:
    ...
    def add_server(self, server_info: dict):
        """Add a server for monitoring"""
        try:
            with self.lock:
                conn = sqlite3.connect(self.db_path)
                cursor = conn.cursor()
                cursor.execute('''
                    INSERT INTO servers (panel_id, ip, port, username, password, ssh_key, node_name)
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                ''', (
                    server_info['panel_id'],
                    server_info['ip'],
                    server_info['port'],
                    server_info['username'],
                    server_info.get('password'),
                    server_info.get('ssh_key'),
                    server_info['node_name']
                ))
                db_server_id = cursor.lastrowid
                conn.commit()
                conn.close()
                return db_server_id
        except Exception as e:
    ...
    def get_servers(self, panel_id: int = None) -> List[Dict[str, Any]]:
        """Get servers for monitoring"""
        try:
            with self.lock:
                conn = sqlite3.connect(self.db_path)
                cursor = conn.cursor()
                if panel_id:
                    cursor.execute('SELECT * FROM servers WHERE panel_id = ?', (panel_id,))
                else:
